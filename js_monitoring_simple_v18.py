        ...
        try:
            ...
            if found_js_count > 0:
                ...
                async with aiohttp.ClientSession() as analysis_session:
                    ...
                    # Запускаем filler в фоне
                    filler_task = asyncio.create_task(queue_filler())

                    # Ждем, пока воркеры обработают все задачи
                    await analysis_queue.join()

                    # Только теперь можно безопасно дождаться завершения filler
                    # (хотя он уже должен быть завершен)
                    await filler_task
