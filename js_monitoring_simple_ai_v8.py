...
# ============================================================================
# ОПТИМИЗИРОВАННОЕ ХРАНИЛИЩЕ ДЛЯ DIFF-АНАЛИЗА (Стратегия 1 + 3)
# ============================================================================

class OptimizedDiffStorage:
    """
    Умное хранилище для JS-файлов с:
    1. zlib-сжатием (экономия 75-85% памяти)
    2. Динамическим TTL (адаптивное время жизни)
    3. Быстрой проверкой изменений по хешу
    """
    ...
    def __init__(self, redis_client: redis.Redis):
        ...
        self.TTL_FREQUENT = 14  # Для часто изменяемых файлов
        self.TTL_MODERATE = 7   # Для умеренно активных
        self.TTL_RARE = 3       # Для редко изменяемых
        
    async def save_js_content(self, url: str, content: str) -> str:
        """
        Сохраняет JS-контент в сжатом виде с умным TTL.
        
        Args:
            url: URL JS-файла
            content: Оригинальное содержимое (НЕ beautified!)
        
        Returns:
            content_hash: SHA256 хеш контента
        """
        # 1. Создаём стабильные ключи
        canonical_url = get_canonical_url(url)
        url_hash = hashlib.md5(canonical_url.encode('utf-8', 'ignore')).hexdigest()
        
        content_key = f"js:compressed:{url_hash}"
        hash_key = f"js:hash:{url_hash}"
        access_key = f"js:access:{url_hash}"
        
        # 2. Сжимаем ОРИГИНАЛЬНЫЙ контент (важно: не beautified!)
        try:
            ...
            # Fallback: сохраняем без сжатия
            compressed = content.encode('utf-8', 'ignore')
        
        # 3. Вычисляем быстрый хеш для детекции изменений
        content_hash = hashlib.sha256(content.encode('utf-8', 'ignore')).hexdigest()
        
        # 4. Определяем умный TTL на основе истории изменений
        try:
            access_count = int(await self.r.get(access_key) or 0)
        except:
            access_count = 0
        
        # Логика динамического TTL:
        if access_count > 5:  # Очень активный файл (изменялся 5+ раз)
            ttl_seconds = 86400 * self.TTL_FREQUENT
        elif access_count > 2:  # Умеренно активный
            ttl_seconds = 86400 * self.TTL_MODERATE
        else:  # Новый или редко изменяемый
            ttl_seconds = 86400 * self.TTL_RARE
        
        # 5. Сохраняем всё в Redis одной транзакцией
        try:
            pipe = self.r.pipeline()
            pipe.set(content_key, compressed, ex=ttl_seconds)
            pipe.set(hash_key, content_hash, ex=ttl_seconds)
            pipe.incr(access_key)  # Увеличиваем счётчик обращений
            pipe.expire(access_key, 86400 * 30)  # Счётчик живёт дольше
            await pipe.execute()
        except Exception as e:
    ...
    async def get_and_compare(self, url: str, new_content: str) -> tuple:
        """
        Главная функция: получает старую версию, создаёт diff, обновляет хранилище.
        ...
        Returns:
            (has_changes, diff, old_hash)
            - has_changes: bool - изменился ли файл
            - diff: str - unified diff (пустой если не изменился)
            - old_hash: str - хеш предыдущей версии
        """
        # 1. Подготовка ключей
        canonical_url = get_canonical_url(url)
        url_hash = hashlib.md5(canonical_url.encode('utf-8', 'ignore')).hexdigest()
        ...
        # 2. Быстрая проверка: изменился ли файл вообще?
        ...
        try:
            ...
            print(f"[!] Redis get hash failed for {url}: {e}", file=sys.stderr)
            old_hash = None
        
        if old_hash and old_hash == new_hash:
            # Контент НЕ изменился - просто продлеваем TTL и выходим
            try:
                await self.r.expire(content_key, 86400 * self.TTL_MODERATE)
        ...
        # 3. Контент ИЗМЕНИЛСЯ - нужно создать diff
        try:
            compressed_old = await self.r.get(content_key)
        except Exception as e:
            print(f"[!] Redis get content failed for {url}: {e}", file=sys.stderr)
            compressed_old = None
        
        if not compressed_old:
            # Это первая версия файла - просто сохраняем без diff
            await self.save_js_content(url, new_content)
            return True, "", ""
        
        # 4. Распаковываем старую версию
        try:
            old_content = zlib.decompress(compressed_old).decode('utf-8', 'ignore')
        except Exception as e:
            print(f"[!] Decompression failed for {url}: {e}", file=sys.stderr)
            # Если не удалось распаковать - считаем что это первая версия
            await self.save_js_content(url, new_content)
            return True, "", old_hash if old_hash else ""
        
        # 5. Создаём красивый diff (beautify применяем ТОЛЬКО для diff!)
        opts = jsbeautifier.default_options()
        opts.indent_size = 2
        ...
        # 7. Сохраняем НОВУЮ версию (в сжатом виде)
        await self.save_js_content(url, new_content)
        
        return True, diff, old_hash if old_hash else ""
    
    async def get_storage_stats(self) -> dict:
        """Возвращает статистику использования хранилища (для мониторинга)."""
        try:
            info = await self.r.info('memory')
            total_keys = await self.r.dbsize()
            
            # Подсчитываем ключи по типам
            compressed_keys = len(await self.r.keys('js:compressed:*'))
            ...
            return {
                ...
                'compressed_content_keys': compressed_keys,
                'hash_keys': hash_keys,
...
def is_framework_template_or_junk(endpoint: str) -> bool:
    """Определяет, является ли строка шаблоном фреймворка или CSS/JS мусором"""
    ep_lower = endpoint.lower()
    
    # Angular/Vue/React templates и JS-объекты
    if any(pattern in ep_lower for pattern in [
        '{{', '}}', '=>', 'this.', '...map', '.component', '.service', 
        'ng-', 'v-', '(event)', '[property]'
    ]):
        return True
    ...
    if any(pattern in ep_lower for pattern in [
        ...
        'margin:', 'padding:', 'border:', 'background:', '!important', 'px', 'rem', 'em'
    ]) and not endpoint.startswith('/'):  # Разрешаем CSS-like слова, если это явный путь
        return True
    
    # Проверка на CSS-подобную структуру: &{...} { ... }
...
async def analyzer_worker_streaming(
    ...
    while True:
        ...
        try:
            ...
            storage = OptimizedDiffStorage(r)
...
async def analyze_orchestrate(args):
    ...
    while True:
        ...
        try:
            ...
            print(f"Peak Memory:       {stats.get('used_memory_peak_human', 'N/A')}")
            print(f"Total Keys:        {stats.get('total_keys', 'N/A')}")
            print(f"Compressed Files:  {stats.get('compressed_content_keys', 'N/A')}")
            print(f"Hash Keys:         {stats.get('hash_keys', 'N/A')}")
            print(f"Fragmentation:     {stats.get('fragmentation_ratio', 'N/A')}")
            print("="*50 + "\n")
