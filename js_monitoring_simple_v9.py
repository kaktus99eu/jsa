        ...
        try:
            async with self.session.get(url, headers=headers, timeout=timeout, ssl=False, allow_redirects=True) as resp:
                ...
                if not is_blocked:
                    ...
                    return content, final_url, False
...
async def hybrid_hybrid_stealth_fetch_content(session: aiohttp.ClientSession, url: str, semaphore: asyncio.Semaphore, 
                                     args, timing, header_manager, hybrid_fetcher: HybridFetcher, referer: str = None):
    """
    Замена для hybrid_stealth_fetch_content с гибридной логикой
    """
    async with semaphore:
        ...
        try:
            content, final_url, used_browser = await hybrid_fetcher.fetch_hybrid(url, headers)
            
            if content:
                # Обновляем состояние сессии только если не использовали браузер
                # (браузер управляет своими cookies автоматически)
                if not used_browser:
                    header_manager.update_session({}, final_url)
                
                if args.debug:
                    method = "BROWSER" if used_browser else "AIOHTTP"
                    print(f"[DEBUG] {method} fetch success for {url}")
                
                return content, final_url
            
        except Exception as e:
            if args.debug:
                print(f"[DEBUG][Hybrid Fetch] Error for {url}: {e}")
    
    return None, url

# Интеграционные функции
async def init_hybrid_system(session: aiohttp.ClientSession) -> HybridFetcher:
    """Инициализация гибридной системы"""
    return HybridFetcher(session)

async def cleanup_hybrid_system(hybrid_fetcher: HybridFetcher):
    """Очистка ресурсов"""
...
async def hybrid_stealth_fetch_content(session: aiohttp.ClientSession, url: str, semaphore: asyncio.Semaphore, args: argparse.Namespace, timing: HumanLikeTiming, header_manager: SmartHeaders, referer: str = None):
    """Стелс версия fetch с WAF evasion"""
    async with semaphore:
        await timing.get_delay()
        headers = header_manager.get_headers(url)
        if referer:
            headers['Referer'] = referer
        try:
            content, final_url, resp_headers = await smart_fetch_with_retry(session, url, headers)
            if content:
                header_manager.update_session(resp_headers, final_url)
                return content, final_url
        except Exception as e:
            if args.debug:
                print(f"[DEBUG][Stealth Fetch] Error for {url}: {e}")
    return None, url

async def crawl_for_js_links(session: aiohttp.ClientSession, base_url: str, semaphore: asyncio.Semaphore, js_queue: asyncio.Queue, pbar_crawl: tqdm, analyzed_js: set, args: argparse.Namespace, timing: HumanLikeTiming, header_manager: SmartHeaders, hybrid_fetcher: HybridFetcher):
    try:
        source_host = urlparse(base_url).hostname
        if not source_host: return
        html_content, final_url = await hybrid_hybrid_stealth_fetch_content(session, base_url, semaphore, args, timing, header_manager, hybrid_fetcher, referer=base_url)
...
async def analyzer_worker_batch(worker_id: int, session: aiohttp.ClientSession, r: redis.Redis, semaphore: asyncio.Semaphore, js_batch: list, all_findings: dict, lock: asyncio.Lock, pbar_analyze: tqdm, args: argparse.Namespace, file_lock: asyncio.Lock, endpoints_file, new_endpoints_file, timing: HumanLikeTiming, header_manager: SmartHeaders, hybrid_fetcher: HybridFetcher):
    ...
    for js_url, source_host, referer_url in js_batch:
        try:
            ...
            content, _ = await hybrid_hybrid_stealth_fetch_content(session, js_url, semaphore, args, timing, header_manager, hybrid_fetcher, referer=referer_url)
            if not content: 
...
async def analyze_orchestrate(args):
    ...
    except Exception as e: sys.exit(f"[!] Redis connection failed: {e}")
    ...
    while True:
        ...
        if args.debug:
            endpoints_file, new_endpoints_file = open("endpoints.txt", "w", encoding="utf-8"), open("new_endpoints.txt", "w", encoding="utf-8")
        
        try:
            crawl_semaphore = asyncio.Semaphore(CONCURRENT_REQUESTS)
            connector = aiohttp.TCPConnector(limit_per_host=2, ssl=False, keepalive_timeout=30, enable_cleanup_closed=True)
            async with aiohttp.ClientSession(connector=connector) as session:
                hybrid_fetcher = await init_hybrid_system(session)
                ...
                # Phase 1: Crawl for JS files
                pbar_crawl = tqdm(total=len(base_urls), desc="Crawling URLs", unit="host", position=0)
                crawler_tasks = [crawl_for_js_links(session, url, crawl_semaphore, js_queue, pbar_crawl, analyzed_js_in_cycle, args, timing, header_manager, hybrid_fetcher) for url in base_urls]
                await asyncio.gather(*crawler_tasks)
                pbar_crawl.close()
                
                # Collect all JS files from queue
                js_files = []
                ...
                pbar_analyze = tqdm(total=len(js_files), desc="Analyzing JS", unit="file", position=1, bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} files analyzed')
                
                # Set progress bar to checkpoint position if resuming
                if checkpoint:
                    ...
                    for worker_id in range(args.threads):
                        ...
                        task = asyncio.create_task(
                            analyzer_worker_batch(
                                ...
                                timing, header_manager, hybrid_fetcher
                            )
                        )
            ...
            await r.close()
            if 'hybrid_fetcher' in locals():  # Добавить эту проверку
                await cleanup_hybrid_system(hybrid_fetcher)
