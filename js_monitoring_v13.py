    ...
    try:
        ...
        if domain_session:
            ...
            if hasattr(hybrid_fetcher, 'browser_handler') and hybrid_fetcher.browser_handler.context:
                try:
                    # Инжектируем куки в уже существующий контекст браузера
                    ...
                    for js_url in js_files_from_spider:
                        found_tasks.add((js_url, source_host, base_url, None))
        ...
        # Добавляем все найденные задачи (от паука или от обычного fetch) в очередь
