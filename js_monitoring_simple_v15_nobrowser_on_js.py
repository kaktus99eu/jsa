...
async def analyzer_worker_streaming(worker_id: int, session: aiohttp.ClientSession, r: redis.Redis,
    ...
    while True:
        ...
        try:
            # 1. Проверяем сигнал завершения (None)
            if task_item is None:
                if args.debug:
                    print(f"[DEBUG][Worker {worker_id}] Shutdown signal received. Exiting.")
                js_queue.task_done()
                break
            ...
            # 2. Используем быстрый фетчер без браузера
            ...
            # 3. Ваш существующий код дедупликации и анализа
            ...
            url_for_log = task_item[0] if task_item else "N/A"
            print(f"\n[!] Worker {worker_id} error processing {url_for_log}: {type(e).__name__}: {e}", file=sys.stderr)
        finally:
            # 4. Сообщаем очереди, что задача обработана, и обновляем прогресс-бар
...
async def analyze_orchestrate(args):
    ...
    while True:
        ...
        try:
            # Увеличим общий семафор, т.к. анализаторы теперь работают быстрее
            crawl_semaphore = asyncio.Semaphore(min(args.threads * 2, 25))
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                try:
                    ...
                    js_queue = asyncio.Queue(maxsize=10000) # Ограничиваем очередь для контроля памяти
                    lock, file_lock, analyzed_js_in_cycle = asyncio.Lock(), asyncio.Lock(), set()
                    
                    # === НОВАЯ ПОТОКОВАЯ АРХИТЕКТУРА ===
                    
                    pbar_analyze = tqdm(desc="Analyzing JS", unit="file", position=1, dynamic_ncols=True)
                    
                    # 1. Запускаем анализаторы СРАЗУ (они будут ждать задач)
                    analyzer_tasks = []
                    for worker_id in range(args.threads):
                        task = asyncio.create_task(
                            analyzer_worker_streaming(
                                worker_id, session, r, crawl_semaphore, js_queue,
                    ...
                    # 2. Запускаем краулеры (они начнут наполнять очередь)
                    ...
                    pbar_crawl.close()
                    print(f"\n[+] Discovery phase complete. Waiting for analyzers to process remaining queue...")
                    
                    # 3. Ждем, пока очередь не будет полностью обработана
                    await js_queue.join()
                    
                    # 4. Теперь, когда очередь пуста, отправляем "сигнал завершения" анализаторам
                    for _ in range(args.threads):
                        await js_queue.put(None)
                    
                    # 5. Ждем, пока все анализаторы корректно завершат свою работу
                    await asyncio.gather(*analyzer_tasks)

                    total_processed = pbar_analyze.n
                    pbar_analyze.total = total_processed
                    pbar_analyze.refresh()
                    pbar_analyze.close()
                    print(f"[+] Analysis phase complete. Processed {total_processed} JS files.")
                    
                    # Чекпоинты больше несовместимы с этой моделью, их нужно убрать или полностью переделать
                    ...
                    # Гарантированно закрываем браузер и выводим статистику
