...
async def analyzer_worker_streaming(
    ...
    while True:
        ...
        try:
            ...
            # ==========================================================
            # === НОВАЯ, ПРАВИЛЬНАЯ ЛОГИКА АНАЛИЗА ===
            # ==========================================================
            try:
            ...
            # Получаем хеш предыдущей версии
            ...
            # ГЛАВНАЯ ПРОВЕРКА: Анализируем ТОЛЬКО если контент изменился
            if last_content_hash != current_content_hash:
                print(f"\n[+] CHANGE DETECTED for: {js_url}")

                # 1. Diff-анализ и отправка в AI (если была старая версия)
                ...
                # 2. Обновляем хеш и тело файла в Redis для будущих сравнений
                pipe = r.pipeline()
                ...
                # 3. Анализ на эндпоинты и ключи (теперь только для измененных файлов)
                await analyze_source_code(content, js_url, source_host)
                
                # 4. Анализ Sourcemap (если есть)
                ...
                # Если контент НЕ изменился, просто пропускаем его
                if args.debug:
