...
async def fetch_js_only(session: aiohttp.ClientSession, url: str, semaphore: asyncio.Semaphore, 
                        args, timing, header_manager, referer: str = None):
    """
    Простой и быстрый фетчер ТОЛЬКО для JS-файлов. Всегда использует aiohttp.
    Никогда не переключается на браузер.
    """
    async with semaphore:
        # Убираем HumanLikeTiming для максимальной скорости при анализе ассетов
        # await timing.get_delay() 
        headers = header_manager.get_headers(url)
        if referer:
            headers['Referer'] = referer
        ...
        try:
            # Одна быстрая попытка. Таймаут можно сделать меньше.
            async with session.get(url, headers=headers, timeout=NORMAL_TIMEOUT, ssl=False, allow_redirects=True) as resp:
                ...
                # Если статус не 200 или Content-Type неверный, просто возвращаем None
                return None
...
async def generate_report_and_notify(findings_dict: dict, args: argparse.Namespace, session: aiohttp.ClientSession, timing: HumanLikeTiming, header_manager: SmartHeaders):
    ...
    # Фильтруем хосты без новых endpoints
    findings_dict = {host: eps for host, eps in findings_dict.items() if eps}
...
# ЗАМЕНИТЕ ВЕСЬ КОД analyzer_worker_streaming НА ЭТОТ:

async def analyzer_worker_streaming(worker_id: int, session: aiohttp.ClientSession, r: redis.Redis,
                                  semaphore: asyncio.Semaphore, js_queue: asyncio.Queue, all_findings: dict,
    ...
    """
    Потоковый воркер-анализатор. Обрабатывает JS-файлы по мере их поступления из очереди.
    Использует ТОЛЬКО быстрый aiohttp фетчер.
    """
    if args.debug:
        print(f"[DEBUG][Worker {worker_id}] Started streaming worker.")
    ...
    while True:
        ...
        if task_item is None:
            js_queue.task_done()  # Mark the None as done
            if args.debug:
                print(f"[DEBUG][Worker {worker_id}] Received termination signal.")
            break
        
        try:
            js_url, source_host, referer_url = task_item
            
            pbar_analyze.set_description(f"Analyzing: {os.path.basename(urlparse(js_url).path)}")
            
            content = await fetch_js_only(
                session, js_url, semaphore, args, timing, header_manager, referer=referer_url
            )
            ...
            if await is_content_processed(r, content_hash):
                if args.debug:
                    print(f"[DEBUG][Worker {worker_id}] Skipping duplicate: {js_url}")
                continue
            ...
            if newly_added:
                async with lock:
                    all_findings[source_host].extend(newly_added)
                if args.debug and new_endpoints_file:
                    async with file_lock:
                        new_endpoints_file.write(f"\n--- New in {js_url} (from {source_host}) ---\n")
                        for ep in sorted(newly_added):
                            new_endpoints_file.write(f"{ep}\n")
            
        except Exception as e:
            print(f"\n[!] Worker {worker_id} error processing {js_url if 'js_url' in locals() else 'N/A'}: {type(e).__name__}: {e}", file=sys.stderr)
        finally:
            # Обновляем прогресс-бар и сообщаем очереди о завершении РОВНО ОДИН РАЗ
            pbar_analyze.update(1)
            js_queue.task_done()
    
    if args.debug:
        print(f"[DEBUG][Worker {worker_id}] Completed all tasks.")
...
async def analyze_orchestrate(args):
    ...
    while True:
        ...
        # Создаем временный файл для очереди JS-файлов
        js_queue_filename = tempfile.mktemp(suffix="_js_queue.txt")
        found_js_count = 0

        # --- НАЧАЛО ГЛАВНОГО БЛОКА TRY ---
        try:
            # === ФАЗА 1: КРАУЛИНГ (с браузером, с записью в файл, BATCH MODE) ===
            print("\n--- Phase 1: Crawling for JS files ---")
            browser_handler = None
            hybrid_fetcher = None
            try:
                BATCH_SIZE = 100  # Обрабатываем по 100 URL за раз
                ...
                async with aiohttp.ClientSession(connector=connector) as session:
                    ...
                    for batch_num in range(total_batches):
                        ...
                        # Инициализируем browser для этого батча
                        ...
                        # КРИТИЧЕСКИ ВАЖНО: Закрываем browser после каждого батча
                ...
                if hybrid_fetcher and browser_handler:
                    ...
                    # Принудительное завершение всех Chromium процессов
                    # В finally блоке Фазы 1, замените существующий код на:
                    try:
                        import psutil
                        killed_count = 0
                        ...
                        def kill_browsers():
                            ...
                            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                                try:
                                    ...
                                    if any(keyword in proc_name or keyword in cmdline_str 
                                           for keyword in ['chromium', 'chrome', 'playwright']):
                        ...
                        killed_count = await asyncio.get_running_loop().run_in_executor(None, kill_browsers)
                        
                        if killed_count > 0:
                            print(f"[CLEANUP] Killed {killed_count} browser processes")
                            await asyncio.sleep(1)
                    except ImportError:
                        print("[WARNING] psutil not installed. Cannot force-kill browsers. Install: pip install psutil")
                    except Exception as e:
                        print(f"[WARNING] Failed to force-kill browsers: {e}")
            ...
            if found_js_count > 0:
                print(f"\n--- Phase 2: Analyzing {found_js_count} JS files ---")
                ...
                async with aiohttp.ClientSession() as analysis_session:
                    ...
                    # 1. (От друга) Убираем лимит очереди для максимальной производительности
                    analysis_queue = asyncio.Queue()

                    # Запускаем воркеры-анализаторы
                    analyzer_tasks = []
                    ...
                    # 2. (Общее) Сначала полностью заполняем очередь
                    print(f"[+] Loading {found_js_count} JS files into analysis queue...")
                    # 3. (От меня) Добавляем надежность через обработку ошибок
                    try:
                        with open(js_queue_filename, 'r', encoding='utf-8') as f:
                            for line in f:
                                line = line.strip()
                                if line: # Пропускаем пустые строки
                                    await analysis_queue.put(json.loads(line))
                    except FileNotFoundError:
                        print(f"[WARNING] JS queue file not found: {js_queue_filename}. Skipping analysis.", file=sys.stderr)
                    
                    # 4. (Общее) Отправляем сигналы завершения для всех воркеров
                    for _ in range(args.threads):
                        await analysis_queue.put(None)
                    
                    # 5. (Общее) Ждем, пока все воркеры завершат работу
