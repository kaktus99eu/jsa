...
async def analyze_orchestrate(args):
    ...
    while True:
        ...
        js_queue_file = tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix="_js_queue.txt", encoding='utf-8')
        ...
        try:
            ...
            if found_js_count > 0:
                ...
                async with aiohttp.ClientSession() as analysis_session:
                    ...
                    # Читаем задачи из файла в очередь
                    ...
                    # Ждем, пока воркеры обработают всю очередь
                ...
                await generate_report_and_notify(all_new_findings_by_host, args, analysis_session, timing, header_manager)

        finally:
            os.remove(js_queue_filename) # Удаляем временный файл
