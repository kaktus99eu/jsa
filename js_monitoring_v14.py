...
@dataclass
class DomainSession:
    """Структура для хранения сессии домена."""
    domain: str
...
class SessionManager:
    """Менеджер для работы с аутентифицированными сессиями."""
    ...
    def _load_health_config(self) -> dict:
        """Загружает конфигурацию для health checks."""
        try:
            with open('health_check_config.json', 'r') as f:
                config = json.load(f)
                print(f"[✅] Loaded health check config for {len(config['health_checks'])} domains")
                return config['health_checks']
        except FileNotFoundError:
            print("[⚠️] health_check_config.json not found, using defaults only")
            return {
                "_default": {
    ...
    async def get_session(self, domain: str) -> Optional[DomainSession]:
        ...
        Сначала ищет точное совпадение (например, 'app.example.com'), 
        а если не находит, ищет сессию для корневого домена ('example.com').
        """
        ...
        # 2. Если не нашли, пытаемся найти для корневого домена
        parts = domain.split('.')
        if len(parts) > 2:
            if len(parts[-2]) <= 3 and len(parts[-1]) <= 2 and len(parts) > 2: # Эвристика для .co.uk, .com.au
                root_domain = '.'.join(parts[-3:])
            else:
                root_domain = '.'.join(parts[-2:])
            
            if root_domain != domain:
                print(f"[ℹ️] No session for '{domain}', trying fallback to root domain '{root_domain}'...")
                session = await self._get_session_by_key(f"session:{root_domain}")
                if session:
                    print(f"[🍪] Found fallback session from '{root_domain}' for '{domain}'")
                    return session
        
        return None

    async def _get_session_by_key(self, session_key: str) -> Optional[DomainSession]:
        """Внутренний helper-метод для извлечения и дешифровки сессии по ключу."""
        ...
        try:
            if self.cipher:
                ...
                # В Redis данные уже могут быть байтами или строкой в зависимости от версии
    ...
    async def mark_expired(self, domain_to_check: str):
        """
        Помечает сессию как expired. Корректно работает с fallback-логикой.
        """
        # 🔥 СНАЧАЛА НАХОДИМ СЕССИЮ, которую нужно пометить
        session = await self.get_session(domain_to_check)
        if session:
            # 🔥 ИСПОЛЬЗУЕМ РЕАЛЬНЫЙ ДОМЕН ИЗ НАЙДЕННОЙ СЕССИИ (session.domain)
            # Это может быть 'yandex.ru', даже если мы проверяли 'music.yandex.ru'
            actual_domain = session.domain 
            
            session_data = {
                'cookies': session.cookies, 'created_at': session.created_at,
                'expires_at': session.expires_at, 'last_validated': session.last_validated,
                'auto_refresh_token': session.auto_refresh_token, 'status': 'expired' # 🔥 Устанавливаем статус
            }
            # Пересохраняем с новым статусом, не меняя TTL
            if self.cipher:
                encrypted = self.cipher.encrypt(json.dumps(session_data).encode())
            ...
            print(f"[⏰] Session for '{actual_domain}' marked as expired (triggered by check on '{domain_to_check}').")
    
    async def list_all_sessions(self) -> List[str]:
        """Возвращает список всех доменов с сохраненными сессиями."""
        pattern = "session:*"
        try:
    ...
    async def check_session_health(self, domain: str, session: DomainSession) -> tuple:
        """
        Проверяет жива ли сессия через реальный HTTP запрос.
        
        Returns:
            (is_alive: bool, reason: str)
        """
        check_config = self.health_config.get(domain, self.health_config.get('_default'))
        
        if not check_config or not check_config.get('url'):
            if time.time() > session.expires_at:
                return False, "Expired by TTL"
            return True, "No health check configured"
        
        domain_to_check = check_config.get('check_domain', domain)
        check_url = f"https://{domain_to_check}{check_config['url']}"
        expected_status = check_config.get('expected_status', 200)
        auth_indicators = check_config.get('auth_indicators', [])
        failure_indicators = check_config.get('failure_indicators', [])
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as http_session:
                async with http_session.get(
                    ...
                    # Проверка 1: Редирект на /login
                    if status in [301, 302, 303, 307, 308]:
                        location = resp.headers.get('Location', '')
                        if 'login' in location.lower() or 'signin' in location.lower():
                            return False, f"Redirect to login: {location}"
                    
                    # Проверка 2: 401/403
                    if status in [401, 403]:
                        return False, f"HTTP {status} - Unauthorized"
                    
                    # Проверка 3: Failure indicators
                    for indicator in failure_indicators:
                        if indicator.lower() in content_lower:
                            return False, f"Found failure indicator: '{indicator}'"
                    
                    # Проверка 4: Успешная auth
                    if status == expected_status:
                        if any(indicator.lower() in content_lower for indicator in auth_indicators):
                            return True, "Health check passed"
                        else:
                            return False, "No auth indicators in response"
...
SCRIPT_BLOCKLIST_DOMAINS = {
    ...
    "scorecardresearch.com", "adobedtm.com", "demdex.net", "muscache.com"
}
...
async def send_diff_to_ai(session, diff, file_path, args, source_page: str = None):  # 🔥 НОВЫЙ ПАРАМЕТР
    """
    Отправляет diff на анализ в server.py.
    🔥 ИЗМЕНЕНИЕ: теперь передает source_page (страницу откуда взят JS файл)
    """
    ...
    # 🔥 НОВОЕ: добавляем source_page в payload
    payload = {
        'diff': diff, 
        'file_path': file_path,
        'base_url': base_url,
        'page_path': page_path,
        'full_url': file_path,
        'source_page': source_page or base_url  # 🔥 НОВОЕ ПОЛЕ - страница-источник JS файла
    }
    ...
    print(f"  🔥 Source Page: {payload['source_page']}")  # 🔥 НОВОЕ
    print(f"  🔥 JS File: {payload['full_url']}")        # 🔥 НОВОЕ
    print(f"{'='*60}\n")
...
# Полностью замени старую функцию на эту новую
async def balanced_stealth_fetch_content(session: aiohttp.ClientSession, url: str, semaphore: asyncio.Semaphore, 
                                       ...
                                       referer: str = None):
    """
    Сбалансированная и отказоустойчивая версия фетчера с внешним тайм-аутом.
    """
    async with semaphore:
        ...
        # 🔥 ШАГ 1: Устанавливаем "железный" сторожевой тайм-аут (например, 90 секунд)
        # Если вся операция (включая fallback на браузер) займет больше времени,
        # она будет принудительно прервана.
        BROWSER_OPERATION_TIMEOUT = 90.0

        try:
            # 🔥 ШАГ 2: Оборачиваем вызов фетчера в asyncio.wait_for()
            content, final_url, used_browser, initial_content = await asyncio.wait_for(
                hybrid_fetcher.fetch_hybrid(url, headers), 
            ...
            if content:
                ...
                if args.debug:
                    ...
                    cache_status = "" # Логика кэша уже внутри fetch_hybrid
                    print(f"[DEBUG] {method} {cache_status} fetch success for {url}")
                
                return content, final_url, initial_content
        
        except asyncio.TimeoutError:
            # 🔥 ШАГ 3: ЛОВИМ ЗАВИСАНИЕ!
            print(f"\n[!!!] CRITICAL: Browser operation for {url} timed out after {BROWSER_OPERATION_TIMEOUT}s. The browser process is likely stuck.", file=sys.stderr)
            print("[RECOVERY] Attempting to restart the browser instance...", file=sys.stderr)
            
            # 🔥 ШАГ 4: ЗАПУСКАЕМ ПРОЦЕДУРУ ВОССТАНОВЛЕНИЯ
            # Мы принудительно "убиваем" старый, возможно сломанный, экземпляр браузера
            # и просим систему создать новый, чистый, на следующей итерации.
            # Эта логика уже есть в твоем основном цикле, здесь мы просто ее форсируем.
            # Эта часть кода предполагает, что `hybrid_fetcher` и его `browser_handler`
            # будут пересозданы для следующего батча.
            # Для немедленного эффекта можно было бы добавить более сложную логику перезапуска,
            # но для твоей батчевой системы это самый простой и надежный способ.
            # Мы просто помечаем эту задачу как проваленную и идем дальше.
            # На следующем батче `init_balanced_hybrid_system` создаст новый браузер.
            
            # Важно! Мы должны корректно обработать эту ошибку, чтобы не уронить весь скрипт.
            # Просто возвращаем None, как будто сайт недоступен.
...
def find_all_js_sources(html_content: str, base_url: str, source_host: str):
    ...
    🔥 ИЗМЕНЕНИЕ: Теперь возвращает dict вместо tuple для сохранения source_page
    """
    if not html_content:
        return []
    ...
    for tag in soup.find_all(['script', 'link']):
        ...
        if src and ('.js' in src or 'javascript' in src):
            ...
            # Проверка по блок-листу
            if any(blocked in urlparse(full_url).netloc for blocked in SCRIPT_BLOCKLIST_DOMAINS):
                continue
            
            # 🔥 НОВЫЙ ФОРМАТ: dict с явным source_page
            tasks_for_queue.append({
                'js_url': full_url,
                'source_host': source_host,
                'source_page': base_url,  # 🔥 НОВОЕ ПОЛЕ - страница откуда взяли JS
                'inline_content': None
            })
    ...
    for script in soup.find_all('script'):
        if not script.get('src'):  # Это inline script
            ...
            if script_content and len(script_content) > 100 and any(
                ...
                # Используем хеш для стабильной идентификации
                script_hash = hashlib.md5(script_content.encode('utf-8')).hexdigest()
                virtual_url = f"{base_url}#inline-script-md5-{script_hash[:16]}"
                
                # 🔥 НОВЫЙ ФОРМАТ: dict с source_page
                tasks_for_queue.append({
                    'js_url': virtual_url,
                    'source_host': source_host,
                    'source_page': base_url,  # 🔥 НОВОЕ - для inline тоже важно знать источник
                    'inline_content': script_content
                })
                
    return tasks_for_queue

# ============================================================================
# 🕷️ AUTHENTICATED SPIDERING
# ============================================================================

class AuthenticatedSpider:
    """Умный паук для обхода authenticated страниц."""
    ...
    def __init__(self, max_pages: int = 20, max_depth: int = 3):
        ...
        self.ignored_patterns = [
            ...
            r'/external/', r'/redirect/',
            r'\.(pdf|zip|jpg|png|gif)$',
            r'/oauth/', r'/saml/'
        ]
        self.compiled_patterns = [re.compile(p, re.IGNORECASE) for p in self.ignored_patterns]
    
    def should_visit(self, url: str, base_domain: str) -> bool:
        """Проверяет, стоит ли посещать этот URL."""
    ...
    async def spider_authenticated_area(self, start_url: str, playwright_context, 
                                       args: argparse.Namespace) -> tuple:
        """Обходит authenticated область сайта."""
        base_domain = urlparse(start_url).netloc
        queue = [(start_url, 0)]
        visited_pages = []
        
        print(f"\n[🕷️] Starting authenticated spider for {base_domain}")
        ...
        while queue and len(self.visited_urls) < self.max_pages:
            ...
            if not self.should_visit(current_url, base_domain):
                continue
            
            self.visited_urls.add(current_url)
            page_data = await self._visit_page(current_url, playwright_context, args)
            ...
            if page_data:
                ...
                self.js_files_found.update(page_data['js_files'])
                
                for link in page_data['links']:
                    if self.should_visit(link, base_domain):
                        queue.append((link, depth + 1))
                
                print(f"    [{len(self.visited_urls)}/{self.max_pages}] "
                      f"Depth {depth}: {current_url[:60]}...")
            
            await asyncio.sleep(random.uniform(1, 2))
        
        print(f"[✅] Spider finished: {len(visited_pages)} pages, "
              f"{len(self.js_files_found)} unique JS files")
        
        return list(self.js_files_found), visited_pages
    
    async def _visit_page(self, url: str, playwright_context, args) -> dict:
        """Посещает одну страницу."""
        page = None
...
async def crawl_for_js_links(session: aiohttp.ClientSession, base_url: str, semaphore: asyncio.Semaphore,
    ...
    try:
        ...
        # ЕСЛИ ЕСТЬ СЕССИЯ - СРАЗУ ЗАПУСКАЕМ ПАУКА
        if domain_session:
            print(f"[🕷️] Authenticated session found, starting deep spider for {source_host}")
            
            if hasattr(hybrid_fetcher, 'browser_handler') and hybrid_fetcher.browser_handler.context:
                try:
                    await hybrid_fetcher.browser_handler.context.add_cookies(domain_session.cookies)
                    
                    spider = AuthenticatedSpider(
                        max_pages=getattr(args, 'spider_max_pages', 20),
                        max_depth=getattr(args, 'spider_max_depth', 3)
                    )
                    
                    js_files_from_spider, visited_pages = await spider.spider_authenticated_area(
                        start_url=base_url,
                        playwright_context=hybrid_fetcher.browser_handler.context,
                        args=args
                    )
                    
                    # 🔥 ИЗМЕНЕНИЕ: создаем dict-задачи вместо tuple
                    for js_url in js_files_from_spider:
                        found_tasks.add(json.dumps({
                            'js_url': js_url,
                            'source_host': source_host,
                            'source_page': base_url,  # 🔥 base_url - это страница откуда начали spider
                            'inline_content': None
                        }))
                    
                    if found_tasks:
                        spider_was_successful = True
                        if args.debug:
                            print(f"[DEBUG] Spider found {len(js_files_from_spider)} JS from {len(visited_pages)} pages.")
                
                except Exception as spider_error:
                    print(f"[!] Spider failed for {base_url}: {spider_error}")
            
        # ЕСЛИ ПАУК НЕ ЗАПУСКАЛСЯ ИЛИ УПАЛ - ИДЕМ ПО СТАРОЙ ЛОГИКЕ
        if not spider_was_successful:
            if domain_session:
                 print("[!] Spider found no JS or failed, falling back to public fetch.")
            ...
            if not final_html_content and not initial_html_content: 
                return

            if initial_html_content:
                tasks = find_all_js_sources(initial_html_content, base_url, source_host)
                for task in tasks: 
                    found_tasks.add(json.dumps(task))  # 🔥 Сериализуем dict
        ...
        # Добавляем все найденные задачи в очередь
        for task_json in found_tasks:
            task = json.loads(task_json)  # 🔥 Десериализуем обратно
            url_to_check = task['js_url']
            
            if url_to_check not in analyzed_urls:
                await js_queue.put(task)  # 🔥 Кладем dict в очередь
...
async def stealth_probe_endpoint_with_auth(session, base_url, endpoint_info, semaphore, 
                                          timing, header_manager, domain_session=None,
                                          playwright_context=None): # 🔥 ДОБАВЛЕН ПАРАМЕТР
    """
    Wrapper для probing с поддержкой authenticated сессий.
    Использует переданный контекст Playwright, если он есть.
    """
    if isinstance(endpoint_info, tuple):
        probe_url, display_name = endpoint_info
    else:
    ...
    async with semaphore:
        ...
        # 🔥 ЕСЛИ ЕСТЬ ГОТОВЫЙ КОНТЕКСТ И СЕССИЯ - используем его
        if playwright_context and domain_session:
            try:
                page = await playwright_context.new_page()
                try:
                    # Используем 'domcontentloaded' для скорости, т.к. нам не нужен полный рендер
                    response = await page.goto(full_url, wait_until='domcontentloaded', timeout=15000)
                    
                    if not response:
                        return display_name, 0, 0, "No response"
                    
                    status = response.status
                    # Получаем длину контента из заголовков, если возможно, это быстрее
                    ...
                    if length == 0:
                        try:
                           length = len(await response.body())
                        except Exception:
                           length = 0 # Оставляем 0 в случае ошибки
                    
                    title = "N/A"
                    # Пытаемся получить title без полного парсинга HTML для скорости
                    ...
                    return display_name, status, int(length), title.strip().replace('\n', ' ').replace('\r', '')[:100]
                ...
                return display_name, 0, 0, f"Browser Error: {type(e).__name__}"
        
        # Если нет сессии или контекста - используем обычный aiohttp
        else:
            headers = header_manager.get_headers(full_url)
            try:
                async with session.get(full_url, headers=headers, timeout=15, 
...
async def generate_report_and_notify(
    ...
    for host in sorted(list(relevant_hosts)):
        ...
        # 🔥 НАЧАЛО НОВОГО БЛОКА: ВОЗВРАЩАЕМ BASELINE PROBING
        print(f"[PROBE] Performing baseline probe for {host}...")
        try:
            # Для baseline probe не нужна аутентификация, используем быстрый aiohttp
            _, status, length, title = await stealth_probe_endpoint(
            ...
            if "Request Error" in title:
                ...
                # Если даже главная страница не открывается, нет смысла проверять остальное
                continue 
            else:
                report_lines.append(f"  BASELINE: / - {title} - {status} - {length}")
        except Exception as e:
            print(f"[!] Baseline probe for {host} crashed: {e}", file=sys.stderr)
            report_lines.append(f"  BASELINE: / - PROBE CRASHED: {type(e).__name__}")
            continue
        
        # 🔥 Проверяем сессию ОДИН РАЗ для хоста
        domain_session = None
        if session_manager and getattr(args, 'enable_auth_mode', False):
        ...
        try:
            # 🔥 ЗАПУСКАЕМ БРАУЗЕР ОДИН РАЗ, ЕСЛИ ЕСТЬ СЕССИЯ
            if domain_session:
                print(f"[BROWSER PROBE] Initializing browser for authenticated probing on {host}")
                playwright_instance = await async_playwright().start()
                browser = await playwright_instance.chromium.launch(headless=True)
                context = await browser.new_context(user_agent=random.choice(REALISTIC_USER_AGENTS))
                ...
                for probe_info in probe_variants:
                    task = asyncio.create_task(
                        stealth_probe_endpoint_with_auth(
                            ...
                            playwright_context=context # 🔥 Передаем готовый контекст
                        )
                    )
            ...
            # 🔥 КОРРЕКТНО ЗАКРЫВАЕМ БРАУЗЕР ПОСЛЕ РАБОТЫ С ХОСТОМ
            if context: await context.close()
            if browser: await browser.close()
            if playwright_instance: await playwright_instance.stop()
            if domain_session:
                print(f"[BROWSER PROBE] Browser for {host} shut down.")
...
async def analyzer_worker_streaming(
    ...
    while True:
        ...
        lock_key = None
        try:
            # 🔥 ИЗМЕНЕНИЕ: task_item теперь dict вместо tuple
            js_url = task_item['js_url']
            source_host = task_item['source_host']
            source_page = task_item['source_page']  # 🔥 НОВОЕ ПОЛЕ
            ...
            if not content:
                fetch_result = await fetch_js_only(
                    session, js_url, semaphore, args, r, referer=source_page  # 🔥 используем source_page как referer
                )
            ...
            # Оптимизированная логика с сжатием
            ...
            if diff.strip() and old_hash:
                ...
                # 🔥 ИЗМЕНЕНИЕ: передаем source_page в send_diff_to_ai
                ai_result = await send_diff_to_ai(
                    session, diff, js_url, args, 
                    source_page=source_page  # 🔥 НОВЫЙ ПАРАМЕТР
                )
                ...
                if ai_result and ai_result.get("signals_found"):
                    for signal in ai_result["signals_found"]:
                        ...
                        if alert_message:
                            ...
                            full_alert += f"\n🔗 **JS File:** {llm_analysis.get('source_url', 'N/A')}"
        ...
        while True:
            try:
                ...
                for domain in all_domains:
                    ...
                    is_alive, reason = await session_manager.check_session_health(domain, session)
                    
                    time_until_expiry = session.expires_at - time.time()
                    days_until_expiry = time_until_expiry / 86400
                    
                    if not is_alive:
                        unhealthy_sessions.append((domain, reason))
                        await session_manager.mark_expired(domain)
                        
                        alert_msg = (
                            f"❌ SESSION EXPIRED\n\n"
                            f"Domain: {domain}\n"
                            f"Reason: {reason}\n\n"
                            f"⚠️ Impact: Missing authenticated JS files\n\n"
                            f"🔧 Action:\n"
                            f"python import_sessions.py --browser chrome --domain {domain}"
                        )
                        await send_notify_alert_async(alert_msg, notify_config)
                        
                    elif days_until_expiry < 3:
                        expiring_soon.append((domain, days_until_expiry))
                        
                        warning_msg = (
                            f"⚠️ SESSION EXPIRING SOON\n\n"
                            f"Domain: {domain}\n"
                            f"Expires in: {days_until_expiry:.1f} days\n\n"
                            f"🔧 Refresh:\n"
                            f"python import_sessions.py --browser chrome --domain {domain}"
                        )
                        await send_notify_alert_async(warning_msg, notify_config)
                    else:
                        healthy_sessions.append(domain)
                
                summary = (
                    f"🔐 SESSION HEALTH\n"
                    f"✅ Healthy: {len(healthy_sessions)}\n"
                    f"⚠️ Expiring: {len(expiring_soon)}\n"
                    f"❌ Unhealthy: {len(unhealthy_sessions)}"
                )
                print(summary)
...
# Запускаем health monitor в фоне (только если не отключен)
