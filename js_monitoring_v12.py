...
class SessionManager:
    ...
    async def save_session(self, domain: str, cookies: List[Dict], 
        ...
        session_data = {
            ...
            'auto_refresh_token': refresh_token,
            'status': 'active'
        }
    ...
    async def get_session(self, domain: str) -> Optional[DomainSession]:
        ...
        try:
            if self.cipher:
                ...
                session_data = json.loads(encrypted_data.decode())
            ...
            print(f"[❌] Failed to decrypt session for {domain}: {e}")
            return None
    
    async def mark_expired(self, domain: str):
        """Помечает сессию как expired."""
        ...
        if session:
            ...
            await self.save_session(
                ...
                session.cookies, 
                expires_in_days=30,
                refresh_token=session.auto_refresh_token
            )
    ...
    async def check_session_health(self, domain: str, session: DomainSession) -> tuple:
        ...
        check_url = f"https://{domain}{check_config['url']}"
