...
async def analyzer_worker_streaming(
    ...
    """
    Воркер, сфокусированный на эндпоинтах и API-ключах.
    Анализирует внешние/внутренние скрипты и sourcemaps с полной дедупликацией.
    """
    ...
    while True:
        ...
        try:
            ...
            # Inline-скрипты всегда считаются "новыми", т.к. их новизна определяется по URL с хешем.
            is_new_content = True 

            if not content:
                # Для внешних JS-файлов используем умный фетчер
                content, is_new_content = await fetch_js_only(
                    session, js_url, semaphore, args, r, referer=referer_url
                )
            
            # Если fetch_js_only вернул 304, content будет None. Пропускаем.
            if not content: 
                continue

            # Двухуровневая дедупликация:
            # Уровень 1 (экономия трафика) уже отработал в fetch_js_only.
            # Уровень 2 (экономия CPU) работает здесь для контента, который был скачан (is_new_content = True).
            if is_new_content:
                content_hash = calculate_content_hash(content)
                if await is_content_processed(r, content_hash):
                    if args.debug:
                        # Исправлена опечатка 'url' на 'js_url'
                        print(f"[DEBUG] Skipping duplicate content (hash match) for {js_url}")
                    continue
                
                await mark_content_processed(r, content_hash)

            # --- ЛИШНИЙ БЛОК ДЕДУПЛИКАЦИИ УДАЛЕН ---

            # Анализируем основной контент (только если он прошел все проверки)
            ...
            if not inline_content and is_new_content:
                ...
                if sourcemap_data:
                    # (остальная логика sourcemap без изменений)
                    if args.debug:
